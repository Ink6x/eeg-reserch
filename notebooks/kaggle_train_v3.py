# ============================================================
# Kaggle Notebook v3: EEGNet + CausalEEGConformer v3
# Dataset: Grasp-and-Lift EEG Detection
#
# v2からの改善点:
#   1. TCNBlock: LayerNorm(transpose) → BatchNorm1d [最重要]
#      transpose-LayerNorm-transposeを廃止。running statsで安定。
#   2. CholeskyPool → LogVarPool (mean + log_var の連結)
#      Cholesky下三角の出力スケール不一致を解消。値域が有界。
#   3. tcn_channels: [32,64,128] → [64,64,128] (ボトルネック削除)
#   4. Classifier: LayerNorm → BatchNorm1d
#   5. forward: last_timestep → GAP
#   6. Conformer LR: 3e-4 → 1e-4, warmup: 5 → 10 epoch
#   7. EEGNet train_step: 20 → 10 (118K窓, 訓練データ2倍)
#
# v2崩壊根本原因:
#   Cholesky修正後もepoch1崩壊継続。
#   原因1: TCNBlock内 transpose-LayerNorm-transpose (running statsなし)
#   原因2: CholeskyPool出力が528次元で対角スケール大 → classifier不安定
# ============================================================
...
class TCNBlock(nn.Module):
    """
    TCN残差ブロック v3
    変更: LayerNorm(transpose) → BatchNorm1d
    - (B, C, T) 形式のまま処理、transpose不要
    - running mean/var でテスト時も安定
    """
...
class LogVarPool(nn.Module):
    ...
    動機:
      CholeskyPool(v2)は下三角要素のスケールが広く(√固有値はO(100)の可能性)、
      528次元の出力でclassifierに巨大なスケール差を生じさせた。
      LogVarPoolは値域が [-3,3] (mean) + [-5,5] (log_var) 程度で有界。

    入力: (B, n_ch, T)
    出力: (B, proj_dim * 2)  — [temporal_mean ; log(temporal_var)]
    """
    def __init__(self, n_ch: int, proj_dim: int = 64, eps: float = 1e-8):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        mean  = x_p.mean(dim=1)                        # (B, proj_dim)
        var   = x_p.var(dim=1, unbiased=False)         # (B, proj_dim)
        log_v = torch.log(var + self.eps)              # (B, proj_dim)
        return torch.cat([mean, log_v], dim=1)         # (B, proj_dim * 2)
...
class CausalEEGConformer(nn.Module):
    ...
    v2からの変更:
      - TCNBlock: LayerNorm(transpose) → BatchNorm1d
      - CholeskyPool(528d) → LogVarPool(128d, 有界スケール)
      - tcn_channels: [32,64,128] → [64,64,128] (ボトルネック除去)
      - classifier: LayerNorm → BatchNorm1d
      - forward: last_timestep → GAP
    """
    def __init__(self, n_channels: int = 32, n_times: int = 250, n_classes: int = 6,
                 tcn_channels = None, tcn_kernel: int = 8, attn_dim: int = 64,
                 stat_proj_dim: int = 64, hidden_dim: int = 256, dropout: float = 0.2):
        super().__init__()
        ...
        combined_dim    = self.stat_pool.out_dim  # stat_proj_dim * 2
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        stats = self.stat_pool(feat)       # (B, stat_proj_dim * 2)
...
class EEGWindowDataset(Dataset):
    def __init__(self, eeg, labels, window_samples=250, step_samples=10,
    ...
    def __getitem__(self, idx):
        ...
        return (self.eeg[end - self.win + 1:end + 1].T.contiguous(),
                self.labels[end])
...
def fit(model, train_eeg, train_labels, val_eeg, val_labels, cfg, save_path):
    ...
    train_ds = EEGWindowDataset(train_eeg, train_labels, win, cfg.get("train_step", 10), 3.0)
...
# ─── セル 8: EEGNet v3 訓練 (train_step=10, 118K窓) ──────────
EEGNET_CFG = {
    "window_samples": 250,
    "train_step":  10,   # v3変更: 20 → 10 (2倍の訓練窓)
    "val_step":    25,
    "batch_size":  512,
    "lr":          5e-4,
    "weight_decay":1e-4,
...
print("=" * 60)
print("[1] EEGNet v3 (within-subject, step=10, ~118K windows)")
print("=" * 60)
...
for subj in range(1, 13):
    ...
    res = fit(EEGNet(N_CHANNELS, 250, N_CLASSES),
              ...
              EEGNET_CFG, MODEL_DIR / f"eegnet_v3_subj{subj}.pt")
...
print(f"\nEEGNet v3  mean AUC: {np.nanmean(eegnet_aucs):.4f} ± {np.nanstd(eegnet_aucs):.4f}")

# ─── セル 9: CausalEEGConformer v3 訓練 ──────────────────────
CONFORMER_CFG = {
    "window_samples": 250,
    "train_step":  20,    # step=20 (59K窓, v2と同条件で公平比較)
    "val_step":    25,
    "batch_size":  512,
    "lr":          1e-4,  # v3変更: 3e-4 → 1e-4
    "weight_decay":1e-4,
    "n_epochs":    100,
    "patience":    15,
    "warmup_epochs": 10,  # v3変更: 5 → 10
    "focal_gamma": 2.0,
    "label_smoothing": 0.05,
}

print("\n" + "=" * 60)
print("[2] CausalEEGConformer v3 (BatchNorm1d + LogVarPool)")
print("=" * 60)
...
for subj in range(1, 13):
    ...
    res = fit(model,
              subject_data[subj]["tr_eeg"], subject_data[subj]["tr_lbl"],
              subject_data[subj]["va_eeg"], subject_data[subj]["va_lbl"],
              CONFORMER_CFG, MODEL_DIR / f"conformer_v3_subj{subj}.pt")
    conformer_aucs.append(res["best_val_auc"])
    conformer_histories.append(res["history"])
    print(f"  >>> Subj {subj:2d}  AUC={res['best_val_auc']:.4f}  ({time.time()-t0:.0f}s)")

print(f"\nConformer v3  mean AUC: {np.nanmean(conformer_aucs):.4f} ± {np.nanstd(conformer_aucs):.4f}")

# ─── セル 10: 結果保存 ───────────────────────────────────────
lr_aucs  = [0.7739,0.8050,0.7152,0.7870,0.6670,0.7436,0.7680,0.7719,0.6729,0.7510,0.6981,0.7019]
v2_eegnet_aucs     = [0.7621,0.7780,0.7337,0.7860,0.7107,0.7242,0.7192,0.7569,0.5460,0.5732,0.7394,0.7608]
v2_conformer_aucs  = [0.6280,0.5560,0.5720,0.6360,0.6050,0.6360,0.5910,0.6230,0.6180,0.5520,0.6160,0.6640]

results = {
    "version": "v3",
    "changes": {
        "TCNBlock":    "LayerNorm(transpose) -> BatchNorm1d",
        "pool":        "CholeskyPool(528d) -> LogVarPool(128d, mean+log_var)",
        "tcn_channels":"[32,64,128] -> [64,64,128]",
        "classifier":  "LayerNorm -> BatchNorm1d",
        "conformer_lr":"3e-4 -> 1e-4",
        "eegnet_step": "20 -> 10 (118K windows)",
    },
    "lr_baseline": {
        "mean_auc": round(np.mean(lr_aucs), 4),
        "per_subject": lr_aucs,
    },
    "eegnet_v3": {
    ...
    "comparison": {
        "eegnet_v2_mean":    round(float(np.nanmean(v2_eegnet_aucs)), 4),
        "conformer_v2_mean": round(float(np.nanmean(v2_conformer_aucs)), 4),
    },
}

out_path = WORK_DIR / "04_dl_results_v3.json"
...
# ─── セル 11: サマリー表示 ───────────────────────────────────
print("\n" + "=" * 60)
print("FINAL RESULTS v3  (within-subject, val series 7-8 holdout)")
print("=" * 60)

print(f"\n{'Subj':>4}  {'LR':>7}  {'EEGNet_v2':>10}  {'EEGNet_v3':>10}  {'Conf_v2':>8}  {'Conf_v3':>8}")
print("-" * 58)
for i in range(12):
    e2 = v2_eegnet_aucs[i]
    e3 = eegnet_aucs[i]
    c2 = v2_conformer_aucs[i]
    c3 = conformer_aucs[i]
    lr = lr_aucs[i]
    delta_e = f"{e3-lr:+.3f}"
    delta_c = f"{c3-lr:+.3f}"
    print(f"  {i+1:2d}   {lr:.4f}   {e2:.4f}   {e3:.4f}{delta_e:>8}   "
          f"{c2:.4f}   {c3:.4f}{delta_c:>8}")
print("-" * 58)
print(f"  Mean {np.mean(lr_aucs):.4f}   {np.nanmean(v2_eegnet_aucs):.4f}"
      f"   {np.nanmean(eegnet_aucs):.4f}"
      f"{np.nanmean(eegnet_aucs)-np.mean(lr_aucs):>+8.3f}"
      f"   {np.nanmean(v2_conformer_aucs):.4f}"
      f"   {np.nanmean(conformer_aucs):.4f}"
      f"{np.nanmean(conformer_aucs)-np.mean(lr_aucs):>+8.3f}")

print(f"\nモデル保存先: {MODEL_DIR}")
print("Output > ダウンロードで 04_dl_results_v3.json と models_v3/*.pt を取得してください。")
